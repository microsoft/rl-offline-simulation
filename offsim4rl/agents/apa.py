...
class APAAgentRevealed(Agent):
    ...
    def _compute_loss_pi(self, transitions):
        ...
        adv_coef = 10
        loss_pi = torch.mean((logp - adv_coef * adv - logp_old) ** 2)

        # Useful extra info
        approx_kl = (logp_old - logp).mean().item()
        ent = pi.entropy().mean().item()
        pi_info = dict(kl=approx_kl, ent=ent, cf=0)
...
class APAAgent(APAAgentRevealed):
    def __init__(
        self,
        *args,
        **kwargs,
    ):
        super(APAAgent, self).__init__(
            *args,
            **kwargs
        )

    def begin_episode(self, observation):
        pi = super(APAAgent, self).begin_episode(observation)
        a = sample_dist(pi)
        self.commit_action(a)
        return a

    def step(self, prev_reward, observation):
        pi = super(APAAgent, self).step(prev_reward, observation)
        a = sample_dist(pi)
        self.commit_action(a)
        return a
