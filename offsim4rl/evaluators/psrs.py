...
class PSRS_Exo(object):
    ...
    def reset_sampler(self, seed=None):
        ...
        # Construct queues based on the latent state of from-state
        self.s_queues = {k: [(s, a, r, s_, *rest) for ((s, _), a, r, (s_, _), *rest) in v] for k,v in itertools.groupby(sorted(self.buffer, key=lambda elt: elt[0][0]), key=lambda elt: elt[0][0])}
